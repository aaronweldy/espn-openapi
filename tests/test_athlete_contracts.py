...
from models.sports_core_api.espn_sports_core_api_client.models.athlete_contracts_response import (
    AthleteContractsResponse,
)
from models.sports_core_api.espn_sports_core_api_client.models.athlete_contract import (
    AthleteContract,
)
...
def test_get_athlete_contracts_nba(sports_core_api_client, ensure_json_output_dir):
    ...
    assert isinstance(result, AthleteContractsResponse), (
        "Response should parse to AthleteContractsResponse"
    )
...
@pytest.mark.api
def test_get_athlete_contract_by_year_nba(
    ...
    result = response.parsed
    assert isinstance(result, AthleteContract), (
        "Response should parse to AthleteContract"
    )

    # Verify contract details
    assert result.ref is not None
    assert result.salary > 0, "Should have a salary"
    assert result.active, "2024 contract should be active"
    assert result.season is not None
    assert result.team is not None

    # Save response for analysis
    with open(
        f"{ensure_json_output_dir}/athlete_contract_nba_lebron_2024.json", "w"
    ) as f:
        json.dump(result.to_dict(), f, indent=2)
...
def test_get_athlete_contracts_cross_sport(
    ...
    response = get_athlete_contracts.sync_detailed(
        client=sports_core_api_client, sport=sport, league=league, athlete_id=athlete_id
    )

    assert response.status_code == 200, (
        f"Expected status code 200, got {response.status_code} for {athlete_name}"
    )

    result = response.parsed
    assert isinstance(result, AthleteContractsResponse), (
        f"Response should parse to AthleteContractsResponse for {athlete_name}"
    )

    # Save response for analysis
    with open(
        f"{ensure_json_output_dir}/athlete_contracts_{league.value}_{athlete_name.lower()}.json",
        "w",
    ) as f:
        json.dump(result.to_dict(), f, indent=2)
