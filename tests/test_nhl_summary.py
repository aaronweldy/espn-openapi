...
from models.site_api.espn_nfl_api_client.models.nhl_game_summary_response import (
...
def test_get_nhl_game_summary(site_api_client, ensure_json_output_dir):
    ...
    response = get_nhl_game_summary.sync_detailed(
        client=site_api_client, event=event_id
    )
    ...
    assert isinstance(result, NhlGameSummaryResponse), (
        "Response should parse to NhlGameSummaryResponse"
    )
    ...
    if result.header:
        header_dict = (
            ...
            if hasattr(result.header, "to_dict")
            else result.header
        )
    ...
    if result.boxscore:
        boxscore_dict = (
            ...
            if hasattr(result.boxscore, "to_dict")
            else result.boxscore
        )
    ...
    if result.game_info:
        game_info_dict = (
            result.game_info.to_dict()
            if hasattr(result.game_info, "to_dict")
            else result.game_info
        )
    ...
    with open(
        f"{ensure_json_output_dir}/nhl_game_summary_{event_id}_test.json", "w"
    ) as f:
            ...
            if competitions and len(competitions) > 0:
                ...
                if "competitors" in competition:
                    ...
                    if home_team and away_team:
                        home_name = home_team.get("team", {}).get(
                            "displayName", "Unknown"
                        )
                        away_name = away_team.get("team", {}).get(
                            "displayName", "Unknown"
                        )
                        print(f"Game: {away_name} @ {home_name}")
                        print(
                            f"Score: {away_team.get('score', '0')} - {home_team.get('score', '0')}"
                        )
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "999999999", marks=pytest.mark.xfail(reason="Invalid event ID should fail")
        ),
    ],
)
def test_get_nhl_game_summary_invalid(site_api_client, event_id):
    ...
    assert response.status_code == 400, (
        f"Expected status code 400 for invalid event, got {response.status_code}"
    )
